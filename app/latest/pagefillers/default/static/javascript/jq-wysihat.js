...
WysiHat.Commands = (function(window) {
  ...
  function toggleOrderedList() {
    ...
    //if (this.orderedListSelected() && !node.is("ol li:last-child, ol li:last-child *")) {
        ...
        // Remove the ol manually, since the toggle of execCommand() does not work 'backwards' on mulitple selected elements (just like the bold or italic command)
    if (this.orderedListSelected()) {
        var prev = node.closest("ol").prev();
        var next = node.closest("ol").next();
        var amountofli = node.closest("ol").find("li").length;
        var list = node.closest("ol"); 
  ...
  function toggleUnorderedList() {
    ...
    //if (this.unorderedListSelected() && !node.is("ul li:last-child, ul li:last-child *")) {
      //selection.selectNode(node.closest("ul"));
       // Remove the ul manually, since the toggle of execCommand() does not work 'backwards' on mulitple selected elements (just like the bold or italic command)
    if (this.unorderedListSelected()) {
        var prev = node.closest("ul").prev();
        var next = node.closest("ul").next();
        var amountofli = node.closest("ul").find("li").length;
