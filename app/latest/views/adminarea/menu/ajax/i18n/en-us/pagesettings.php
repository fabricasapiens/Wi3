...
<?php 
    ...
    echo "<h2>View and edit page</h2>";
    echo "<p><label for='link'>links: </label><span name='viewlink'><a target='_blank' href='" . Wi3::inst()->urlof->page($page) . "'>view</a></span> <span name='editlink'><a href='" . Wi3::inst()->urlof->action("adminarea", "content") . "_" . $page->id . "'>edit</a></span></p>";
    ...
    try 
    {
        ...
        echo "<p><label for='visible'>visible in menu: </label><select name='visible' id='visible' value='" . $visible . "'>" . Wi3::inst()->optionlist(array("1"=>"visible", "0"=>"hidden"),$visible) . "</select></p>";
    } 
    catch(Exception $e) 
    {
        // Well, no edit-rights obviously
        echo "<p>You don't have the proper rights to edit page settings.</p>";
    }
    ...
    try 
    {
        ...
        echo "<p>
        <label for='viewright'>for viewing: </label><input name='viewright' id='viewright' type='text' value='" . $page->viewright . "' /><br />
        <label for='editright'>for editing: </label><input name='editright' id='editright' type='text' value='" . $page->editright . "' /><br />
        <label for='adminright'>for admin: </label><input name='adminright' id='adminright' type='text' value='" . $page->adminright . "' /><br />
        </p>";
    }
    catch(Exception $e) 
    {
    ...
    //opslaan knop
    echo " <button onClick='wi3.request(\"adminarea_menu_ajax/editPageSettings/\" + $(\"#wi3_edited_page\").html(), $(\"#wi3_pageedit_form\").serializeArray());'>save</button>";
    ...
    {
        echo "<p>You don't have the proper rights to edit template settings</p>";
    }
    echo "</form>";
    //save button
    echo " <button onClick='wi3.request(\"pagefiller_default_ajax/editPageTemplateSettings/\" + $(\"#wi3_edited_page\").html(), $(\"#wi3_pageedit_template_form\").serializeArray());'>save</button>";
    ...
    echo "<h2>Redirects</h2>";
    try 
    {
        Wi3::inst()->acl->check($page);
        echo "<p>";
        echo "<select style='width: 300px;' name='redirect_type' id='redirect_type' value='" . $page->redirecttype . "'>" . Wi3::optionlist(array("none"=>"No redirect; current page content", "wi3" => "Redirect to page within site", "external"=>"Redirect to external URL"), $page->redirecttype) . "</select><br />";
        ...
        */
        ...
        echo "<label for='redirect_wi3'>Redirect to page </label><select name='redirect_wi3' value='" . $page->redirect_wi3 . "'>" . Wi3::optionlist( $pagelist, $page->redirect_wi3) . "</select></p>";
        echo "<p id='redirect_external' " . ($page->redirecttype!="external"?" style='display:none;'":"") . ">";
        echo "<label for='redirect_external'>Redirect to URL </label><input type='text' name='redirect_external' value='" . $page->redirect_external. "' />";
        echo "</p>";
    }
    catch(Exception $e) 
    {
        throw($e);
        echo "<p>You don't have the proper rights to set a redirect.</p>";
    }
    echo "</form>";
    //save button
