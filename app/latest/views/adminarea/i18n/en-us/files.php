<?php
    ...
    echo "<div id='wi3_prullenbak'>";
        echo "<div id='prullenbak_onder'><h2>Drag here to remove file</h2></div>";
    echo "</div>";
    echo "<div id='wi3_add_file'>";
        if (isset($message)) { echo "<div style='color: #cc0000; padding: 15px;'>" . $message . "</div>"; }
        echo "<a href='javascript:void(0);' onClick='$(this).next().slideToggle();'><h2>Add file</h2></a>";
    ...
    if (count($files) > 0)
    {
        ...
        foreach($files as $file)
        {
            ...
            echo "<span>" . html::anchor(Wi3::inst()->urlof->site . "_uploads/" . $file->filename, ($file->isImage() ? "<img src='" . Wi3::inst()->urlof->site . "_uploads/30/" . $file->filename . "'/> " : "") . $file->title) . "</span>";
        }
