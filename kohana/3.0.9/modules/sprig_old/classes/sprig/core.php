				...
				if ($field instanceof Sprig_Field_HasMany)
				{
					...
					else
					{
						...
						else
						{
							...
							{
                                // Changed by Willem Mulder
                                // was $fk = $model->fk(); 
                                // FK is constructed as $a->_model."_".$a->primary_key
                                // which would return the foreign key of the foreign model (i.e. othermodel_id)
                                // while we have a HasMany field that wants to search the foreign model on the column that refers to this model (thus {$this->model}."_id")
                                // 
								$fk = $model->fk();
							}
